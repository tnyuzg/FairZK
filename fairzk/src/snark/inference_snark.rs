use crate::piop::{InferenceIOP, InferenceInstance, LookupIOP};
use crate::piop::{LookupInstance, VectorLookupIOP};

use crate::pcs::OracleTable;
use crate::piop::VectorLookupInstance;

use algebra::PrimeField;
use algebra::{utils::Transcript, AbstractExtensionField, Field};
use pcs::utils::{
    code::{LinearCode, LinearCodeSpec},
    hash::Hash,
};

use serde::{Deserialize, Serialize};

#[derive(Default)]
pub struct InferenceSnark<F, EF, H, C, S>
where
    F: Field + Serialize + for<'de> Deserialize<'de>,
    EF: AbstractExtensionField<F> + Serialize + for<'de> Deserialize<'de>,
    H: Hash + Sync + Send,
    C: LinearCode<F> + Serialize + for<'de> Deserialize<'de>,
    S: LinearCodeSpec<F, Code = C> + Clone,
{
    pub infairence_iop: InferenceIOP<F, EF, H, C, S>,
    pub lookups: Vec<LookupIOP<F, EF, H, C, S>>,
    pub vectorlookups: Vec<VectorLookupIOP<F, EF, H, C, S>>,
}

impl<F, EF, H, C, S> InferenceSnark<F, EF, H, C, S>
where
    F: PrimeField + Serialize + for<'de> Deserialize<'de>,
    EF: AbstractExtensionField<F> + Serialize + for<'de> Deserialize<'de>,
    H: Hash + Sync + Send,
    C: LinearCode<F> + Serialize + for<'de> Deserialize<'de>,
    S: LinearCodeSpec<F, Code = C> + Clone,
{
    pub fn proof_size(&self) -> usize {
        let mut proof_size = 0;
        proof_size += self.infairence_iop.proof_size();
        self.lookups.iter().for_each(|x| {
            proof_size += x.proof_size();
        });
        self.vectorlookups.iter().for_each(|x| {
            proof_size += x.proof_size();
        });
        proof_size
    }

    pub fn prove(
        &mut self,
        instance: &mut InferenceInstance<F>,
        oracle_table: &mut OracleTable<F, EF, H, C, S>,
        trans: &mut Transcript<EF>,
    ) {
        let lookup_instances = instance.lookup_instances();
        let vector_lookup_instances = instance.vector_lookup_instances();

        instance.construct_oracles(oracle_table);

        lookup_instances
            .iter()
            .for_each(|lookup| lookup.construct_first_oracles(oracle_table));

        vector_lookup_instances
            .iter()
            .for_each(|lookup| lookup.construct_first_oracles(oracle_table));

        let mut lookup_instances_ef: Vec<LookupInstance<EF, F, EF>> = lookup_instances
            .iter()
            .map(|lookup| lookup.to_ef())
            .collect();

        let mut vector_lookup_instances_ef: Vec<VectorLookupInstance<EF, F, EF>> =
            vector_lookup_instances
                .iter()
                .map(|lookup| lookup.to_ef())
                .collect();

        lookup_instances_ef
            .iter_mut()
            .for_each(|lookup| lookup.construct_second_oracle(oracle_table, trans));

        vector_lookup_instances_ef
            .iter_mut()
            .for_each(|lookup| lookup.construct_second_oracle(oracle_table, trans));

        let mut instance_ef = instance.to_ef();

        self.infairence_iop
            .prove(&mut instance_ef, oracle_table, trans);

        lookup_instances_ef.iter().for_each(|lookup_instance| {
            let mut lookup = LookupIOP::default();
            lookup.prove(lookup_instance, oracle_table, trans);
            self.lookups.push(lookup);
        });

        vector_lookup_instances_ef
            .iter()
            .for_each(|lookup_instance| {
                let mut lookup = VectorLookupIOP::default();
                lookup.prove(lookup_instance, oracle_table, trans);
                self.vectorlookups.push(lookup);
            });

        oracle_table.prove(trans);
    }

    pub fn verify(
        &self,
        oracle_table: &OracleTable<F, EF, H, C, S>,
        trans: &mut Transcript<EF>,
    ) -> bool {
        self.lookups
            .iter()
            .all(|lookup| lookup.pre_randomness(trans))
            && self
                .vectorlookups
                .iter()
                .all(|lookup| lookup.pre_randomness(trans))
            && self.infairence_iop.verify(oracle_table, trans)
            && self
                .lookups
                .iter()
                .all(|lookup| lookup.verify(oracle_table, trans))
            && self
                .vectorlookups
                .iter()
                .all(|lookup| lookup.verify(oracle_table, trans))
            && oracle_table.verify(trans)
    }

    pub fn prove_timing(
        &mut self,
        instance: &mut InferenceInstance<F>,
        oracle_table: &mut OracleTable<F, EF, H, C, S>,
        trans: &mut Transcript<EF>,
    ) {
        let lookup_instances =
            timing!("construct lookup instance", { instance.lookup_instances() });
        let vector_lookup_instances = timing!("construct vector lookup instance", {
            instance.vector_lookup_instances()
        });

        timing!("commit inference oracles", {
            instance.construct_oracles(oracle_table);
        });

        timing!("commit lookup first oracles", {
            lookup_instances
                .iter()
                .for_each(|lookup| lookup.construct_first_oracles(oracle_table));
        });

        timing!("commit vector lookup first oracles", {
            vector_lookup_instances
                .iter()
                .for_each(|lookup| lookup.construct_first_oracles(oracle_table));
        });

        let mut lookup_instances_ef: Vec<LookupInstance<EF, F, EF>> = lookup_instances
            .iter()
            .map(|lookup| lookup.to_ef())
            .collect();

        let mut vector_lookup_instances_ef: Vec<VectorLookupInstance<EF, F, EF>> =
            vector_lookup_instances
                .iter()
                .map(|lookup| lookup.to_ef())
                .collect();

        timing!("commit lookup second oracles", {
            lookup_instances_ef
                .iter_mut()
                .for_each(|lookup| lookup.construct_second_oracle(oracle_table, trans));
        });

        timing!("commit vector lookup second oracles", {
            vector_lookup_instances_ef
                .iter_mut()
                .for_each(|lookup| lookup.construct_second_oracle(oracle_table, trans));
        });

        let mut instance_ef = instance.to_ef();

        timing!("prove inference sumcheck", {
            self.infairence_iop
                .prove(&mut instance_ef, oracle_table, trans);
        });

        timing!("prove lookup sumcheck", {
            lookup_instances_ef.iter().for_each(|lookup_instance| {
                let mut lookup = LookupIOP::default();
                lookup.prove(lookup_instance, oracle_table, trans);
                self.lookups.push(lookup);
            });
        });

        timing!("prove vector lookup sumcheck", {
            vector_lookup_instances_ef
                .iter()
                .for_each(|lookup_instance| {
                    let mut lookup = VectorLookupIOP::default();
                    lookup.prove(lookup_instance, oracle_table, trans);
                    self.vectorlookups.push(lookup);
                });
        });

        timing!("prove oracles", {
            oracle_table.prove(trans);
        });
    }

    pub fn verify_timing(
        &self,
        oracle_table: &OracleTable<F, EF, H, C, S>,
        trans: &mut Transcript<EF>,
    ) -> bool {
        self.lookups
            .iter()
            .all(|lookup| lookup.pre_randomness(trans))
            && self
                .vectorlookups
                .iter()
                .all(|lookup| lookup.pre_randomness(trans))
            && timing!("verify infairence sumcheck", {
                self.infairence_iop.verify(oracle_table, trans)
            })
            && timing!("verify lookup sumcheck", {
                self.lookups
                    .iter()
                    .all(|lookup| lookup.verify(oracle_table, trans))
            })
            && timing!("verify vector lookup sumcheck", {
                self.vectorlookups
                    .iter()
                    .all(|lookup| lookup.verify(oracle_table, trans))
            })
            && timing!("verify oracle", { oracle_table.verify(trans) })
    }
}
