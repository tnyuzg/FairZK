...
use crate::piop::{LookupIOP, SpectralNormIOP, SpectralNormInstance};
...
#[derive(Default)]
pub struct SpectralNormSnark<F, EF, H, C, S>
where
...
{
    pub spectral_norm_iop: SpectralNormIOP<F, EF, H, C, S>,
    pub lookups: Vec<LookupIOP<F, EF, H, C, S>>,
}

impl<F, EF, H, C, S> SpectralNormSnark<F, EF, H, C, S>
where
    ...
    S: LinearCodeSpec<F, Code = C> + Clone,
{
    pub fn proof_size(&self) -> (usize, usize, usize, usize) {
        let (square, eigen) = self.spectral_norm_iop.proof_size();
        let rangecheck_err = self.lookups[0].proof_size();
        let rangecheck_eig = self.lookups[1].proof_size();
        (square, eigen, rangecheck_err, rangecheck_eig)
    }

    pub fn prove(
        &mut self,
        instance: &mut SpectralNormInstance<F>,
        oracle_table: &mut OracleTable<F, EF, H, C, S>,
        trans: &mut Transcript<EF>,
    ) {
        let lookup_instances: Vec<LookupInstance<F, F, EF>> =
    ...
    pub fn verify(
        ...
        self.lookups
            ...
            && timing!("verify spectral norm sumcheck", {
                self.spectral_norm_iop.verify(oracle_table, trans)
            })
