...
impl<F: Field + Serialize> MLSumcheck<F> {
    ...
    pub fn verify_with_initial_trans(
        polynomial_info: &PolynomialInfo,
        claimed_sum: F,
        proof: &Proof<F>,
    ) -> Result<SumcheckOracle<F>, Error> {
        let mut trans = Transcript::<F>::new();
        Self::verify_old(&mut trans, polynomial_info, claimed_sum, proof)
    }
    ...
    pub fn verify(
        ...
        trans.append_message(b"polynomial info", &proof.info);
        let mut verifier_state = IPForMLSumcheck::verifier_init(&proof.info);
        for i in 0..proof.info.num_variables {
            let prover_msg = proof.proof.get(i).expect("proof is incomplete");
            trans.append_message(b"sumcheck msg", prover_msg);

            IPForMLSumcheck::verify_round((*prover_msg).clone(), &mut verifier_state, trans);
        }
        IPForMLSumcheck::check_and_generate_subclaim(verifier_state, proof.claimed_sum)
    }
}
