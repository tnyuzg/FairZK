...
use fairzk::sumcheck::IPForMLSumcheck;
use fairzk::sumcheck::MLSumcheck;
use rand::prelude::*;
...
fn test_protocol<F: Field + Serialize>(
    ...
    assert!(
        poly.evaluate(&subclaim.sumcheck_point) == subclaim.oracle_eval,
        "wrong subclaim"
    );
}
...
fn test_polynomial(nv: usize, num_multiplicands_range: (usize, usize), num_products: usize) {
    ...
    let poly_info = poly.info();
    let proof = MLSumcheck::prove_with_initial_trans(&poly).expect("fail to prove");
    let subclaim = MLSumcheck::verify_with_initial_trans(&poly_info, asserted_sum, &proof)
...
fn test_polynomial_as_subprotocol<F: Field + Serialize>(
    ...
    let subclaim = MLSumcheck::verify_old(verifier_trans, &poly_info, asserted_sum, &proof)
        .expect("fail to verify");
    assert!(
        poly.evaluate(&subclaim.sumcheck_point) == subclaim.oracle_eval,
        "wrong subclaim"
    );
    assert_eq!(prover_state.randomness, subclaim.sumcheck_point);
}
