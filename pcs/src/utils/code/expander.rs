...
impl ExpanderCodeSpec {
    ...
    pub fn codeword_len(&self, message_len: usize) -> usize {
        if message_len <= ceil(self.r * self.recursion_threshold as f64) {
            return ceil(self.r * message_len as f64);
        }
...
impl<F: Field> ExpanderCode<F> {
    ...
    pub fn new(
        ...
        if message_len <= ceil(spec.r * spec.recursion_threshold as f64) {
            let r = spec.r;

            Self {
                spec,
                message_len,
                codeword_len: ceil(r * message_len as f64),
                a: vec![],
                b: vec![],
            }
        } else {
...
impl<F: Field> LinearCode<F> for ExpanderCode<F> {
    ...
    fn encode(&self, target: &mut [F]) {
        ...
        if self.message_len <= ceil(self.spec.r * self.spec.recursion_threshold as f64) {
            let reedsolomon_code =
                ReedSolomonCode::new(self.message_len, self.codeword_len - self.message_len);
            reedsolomon_code.encode(&mut target[self.message_len..]);
            return;
        }
        ...
        EF: algebra::AbstractExtensionField<F>,
    {
        ...
        if self.message_len <= ceil(self.spec.r * self.spec.recursion_threshold as f64) {
            let reedsolomon_code =
                ReedSolomonCode::new(self.message_len, self.codeword_len - self.message_len);
            reedsolomon_code.encode_ext(&mut target[self.message_len..]);
            return;
        }
